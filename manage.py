...
from lugcampkasse import app, db, User, Bill, BillEntry, Voucher, ShopItem, socketio
...
def import_csv(csv_filename, init_db=False):
    ...
    reader = tablereader.TableReader(csv_filename, force_type="unicodecsv")
    for row in reader:
...
def runserver():
    ...
    socketio.run(app, host=app.config['BIND_IP'], port=app.config['HTTP_PORT'])
